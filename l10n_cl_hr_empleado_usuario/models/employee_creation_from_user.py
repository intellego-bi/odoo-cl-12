            ...
            if len(partner_ids) == 1:
                for partner in partner_ids:
                    ...
                    if self.user_type == 'empl':
                        self.partner_id.write({'employee': bool_emp, 'user_id': self.id, 'ref': self.identification_id})
                        return
                    else:
                        self.partner_id.write({'employee': bool_int, 'user_id': self.id, 'ref': self.identification_id})
                        return
            else:
